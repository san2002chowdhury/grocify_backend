export const confirmOrderTemplate = (userName, order) => {
      ...
      <tr>
        <td style="padding:40px 30px;color:#333333;">
          ...
          <p style="font-size:15px;line-height:1.6;margin:0 0 20px;">
            Your order has been placed successfully. Please keep cash ready at delivery time.
          </p>
          <div style="background:#ffffff;padding:15px;border-radius:8px;border:1px solid #e5e7eb;margin-bottom:25px;">
            <p style="margin:0 0 8px;font-size:14px;"><b>Order ID:</b> ${order?._id?.toString()?.slice(-6)}</p>
            <p style="margin:0 0 8px;font-size:14px;"><b>Order Date:</b> ${new Date(
    order.createdAt,
  ).toDateString()}</p>
            <p style="margin:0;font-size:14px;">
              <b>Shipping Address:</b>
              ${order.shippingAddress.landmark}, ${order.shippingAddress.city}, <br/> ${order.shippingAddress.state
    }
            </p>
          </div>
